...
pub fn apply_prefab(
    ...
    let dijistra_map = DijkstraMap::new(
        SCREEN_WIDTH,
        SCREEN_HEIGHT,
    ...
    while placement.is_none() && attempts < 10 {
        ...
        dimensions.for_each(|pt| {
            ...
            let distance = dijistra_map.map[idx];
        ...
        if can_place {
            placement = Some(
                Point::new(dimensions.x1, dimensions.y1)
            );
    ...
    if let Some(placement) = placement {
        let string_vec: Vec<char> = FORTRESS.0.chars().filter(|a| *a != '\n').collect();
        let mut i = 0;
